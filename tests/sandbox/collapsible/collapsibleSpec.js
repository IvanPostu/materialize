function waitSync(seconds) {
  ...
  const targetTime = startTime + seconds * 1000; // Convert seconds to milliseconds

  while (currentTime < targetTime) {
    currentTime = Date.now();
    // You can add a small delay here to avoid tight loop and reduce CPU usage
    // For example:
    // while (Date.now() < currentTime + 10) { /* Do nothing */ }
  }
}
